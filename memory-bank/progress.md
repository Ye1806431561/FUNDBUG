...
## 2026-02-12 - 维护: 更新 .gitignore 并清理 GitHub 远程仓库 ✅

### 完成内容

1.  **修改 .gitignore**：
    -   将 `prompts/` 文件夹添加到 `.gitignore` 中，确保 AI 提示词文件不会被错误地提交。
2.  **清理远程仓库**：
    -   在 `main` 分支上合并了 `.gitignore` 的更改并推送到 GitHub。
    -   运行 `git rm -r --cached prompts/` 将已提交的 `prompts/` 文件夹从 Git 索引中移除。
    -   推送索引变更到 GitHub，成功从远程仓库中删除了该文件夹，同时保留了本地副本。

### 关键决策

1.  **保持本地副本** — 使用 `--cached` 参数确保仅删除索引中的记录，而不会物理删除本地磁盘上的 `prompts/` 文件夹。
2.  **分支同步** — 确保更改在 `main` 分支上生效，以维护生产环境配置的简洁性。

### 验证结果

-   `git check-ignore -v prompts/test.txt` 返回忽略规则信息，证明 `.gitignore` 生效。
-   远程仓库确认 `prompts/` 文件夹已消失。
