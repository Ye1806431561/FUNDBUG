...
## 2026-02-12 - 步骤 5.2: 完善测试基础设施 ✅

### 完成内容

1.  **全局测试夹具优化**：
    -   在 `tests/conftest.py` 中添加了 `app` (FastAPI) 和 `client` (Fast-API TestClient) 的全局 fixtures。
    -   使得 API 层测试无需重复初始化客户端，提升了测试代码的简洁性和一致性。
2.  **测试脚本封装**：
    -   创建了 `run_tests.sh` 脚本，封装了 `PYTHONPATH=.` 的环境变量设置及 `pytest` 命令。
    -   实现了“一键运行全量测试”的功能。
3.  **代码清理**：
    -   重构了 `tests/test_api.py`，移除了冗余的 `app` 和 `client` 定义，改用全局 fixture 注入。

### 验证结果

```bash
$ ./run_tests.sh
Running all tests with PYTHONPATH=.
============================== 46 passed in 3.19s ==============================
Exit code: 0 ✅
```
...
-   以后编写新的 API 测试时，直接在测试函数参数中声明 `client` 即可自动获取 FastAPI TestClient。
-   所有涉及数据库的测试都会自动使用临时 SQLite 数据库，测试结束后自动清理。
...
> **阶段 5（主入口与调度）已全部完成，可以开始阶段 6（前端界面）。根据用户指令，在验证测试前不要开始 6.1。**

---

## 2026-02-12 - 步骤 6.1: 创建前端主页面 (index.html) ✅

### 完成内容

1. **重构 `frontend/index.html`**：
    - 建立了语义化的 HTML5 结构，包括 `header`、`section`(添加表单)、`main`(关注列表容器) 和 `footer`。
    - 引入了 `style.css` 样式表和 `app.js` 脚本。
    - 实现了输入框正则校验 (`pattern="^\d{6}$"`)。
2. **创建 `frontend/style.css`**：
    - 采用现代暗色模式设计系统。
    - 实现了网格布局（Grid）以展示基金卡片，并定义了涨跌色值（红涨绿跌）。

### 验证结果

- [x] 确认 HTML 结构包含所有必需 ID (`watchlist-container`, `add-fund-form` 等)。
- [x] 确认 CSS 实现了响应式布局，且样式命名符合语义。
